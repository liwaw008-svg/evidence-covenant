...
def parsed_source(value):
 ...
 if p.scheme.lower()!='https' or not p.hostname or p.username or p.password or p.fragment:raise gl.vm.UserError(EXPECTED+' valid HTTPS source required')
 host=p.hostname.lower().rstrip('.')
 try:port=p.port
 except:raise gl.vm.UserError(EXPECTED+' valid HTTPS source required')
 origin='https://'+host+((':'+str(port)) if port and port!=443 else '')
 path=unquote(p.path or '/')
 if any(x in ('.','..') for x in path.split('/')):raise gl.vm.UserError(EXPECTED+' normalized source path required')
...
class EvidenceCovenant(gl.Contract):
 ...
 def create_covenant(self,i:str,question:str,requirements:list[str],outcomes:list[str],identical_content_outcome:str,authorized_origins:list[str])->None:
  k=clean(i,64);rs=[clean(x,300) for x in requirements[:12] if clean(x,300)];os=[clean(x,40).upper() for x in outcomes[:8] if clean(x,40)];orig=[clean(x,500) for x in authorized_origins[:8]]
  origin_keys=[parsed_source(x) for x in orig]
  ...
  if len(set(os))!=len(os) or len(set(origin_keys))!=len(origin_keys) or exact not in os:raise gl.vm.UserError(EXPECTED+' outcomes and origins must be distinct and exact outcome allowed')
 ...
 def _judge(self,c,urls):
  def run():
   ...
   for u in urls:
    ...
    raw=r.body;digests.append(hashlib.sha256(raw).hexdigest());records.append(clean(raw.decode('utf-8'),2400))
 ...
 def resolve(self,i:str,sources:list[str])->None:
  ...
  used=[]
  for url in urls:
   matches=[index for index,rule in enumerate(orig) if matches_slot(rule,url)]
   if len(matches)!=1 or matches[0] in used:raise gl.vm.UserError(EXPECTED+' source must match one distinct authorized HTTPS slot')
